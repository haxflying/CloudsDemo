	...
	SubShader
	{
		...
		Pass
		{
			...
			float getAttenuation(float3 p, float3 lightDir)
			{
				...
				return min(0.25, saturate(1 - atten * 20));
			...
			fixed4 frag (v2f i) : SV_Target
			{
			    ...
			    float4 vlight = rayTrace(_WorldSpaceCameraPos, normalize(ray), dist) * 2;
			    vlight *= _LightColor0;
			    //vlight = lerp(-0.2, 3, vlight);
			    //vlight = pow(vlight * 2, 2) * 50;
			    //vlight = min(0.8, saturate(vlight));
			   	//return vlight;
